...
def temporary_directionOfEffect(evidences, datasource_filter,mecact,target):
    ...
    """
    evidences=evidences.filter(F.col("datasourceId").isin(datasource_filter))
    ...
    assessment = (
        ...
        .withColumn(
            "variantEffect",
            F.when( ### new since we removed the first when 
...
def build_gwasResolvedColoc(path):
    ...
    diseases = spark.read.parquet(f"{path}disease/") ### (f"{path}diseases/") for the previous path (24.12)
