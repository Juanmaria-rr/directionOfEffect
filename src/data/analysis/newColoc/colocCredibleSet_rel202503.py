...
pathN="gs://open-targets-pre-data-releases/2503-testrun-1/output/"

target = spark.read.parquet(f"{pathN}target/")

diseases = spark.read.parquet(f"{pathN}disease/")

evidences = spark.read.parquet(f"{pathN}evidence")

credible = spark.read.parquet(f"{pathN}credible_set/")
...
studyindex= spark.read.parquet(f"{pathN}/study")

new = spark.read.parquet(f"{pathN}/colocalisation/coloc")

# variantIndex = spark.read.parquet(f"{pathN}/variantIndex")

biosample = spark.read.parquet(f"{pathN}/biosample")
...
newColoc = (
    ...
    .join(
    studyindex.selectExpr(  ### bring modulated target on right side (QTL study)
        "studyId as rightStudyId",
        "geneId",
        "projectId",
...
def process_pivoted_data(df_name):
    ...
    rows = comparisons_df_iterative(df.columns[14:16])  # Get row comparisons
