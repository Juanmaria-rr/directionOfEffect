...
evidences_2412 = spark.read.parquet(f"{path}evidence").filter(F.col("datasourceId")!="chembl")

### new chembl evidences where phase 4 is clean
evidences_chembl = spark.read.parquet("gs://open-targets-pre-data-releases/2503-testrun-1/output/evidence/sourceId=chembl/")

evidences = evidences_2412.unionByName(evidences_chembl,allowMissingColumns=True) 
...
target_path = f"{path}targets/" ### target_path = f"{path}targets/" for the previous path (24.12
mecact_path = f"{path}mechanismOfAction/" ### f"{path}mechanismOfAction/" for the previous path (24.12)

target = spark.read.parquet(target_path)
mecact = spark.read.parquet(mecact_path)

assessment, evidences, actionType, oncolabel = temporary_directionOfEffect(
    evidences, datasource_filter,mecact,target
)
...
def aggregations_original(
    ...
    print("wComparison is", wComparison)
    print("wPrediction is",wPrediction)
    print("wPredictionComparison",wPredictionComparison)
    results = []
...
for col in variables_study:
    ...
    pivoted_df = (
        ...
        .agg(
            F.collect_set(F.when(F.col("rightTissue") == "yes", F.col("agree_lowestPval")).otherwise(F.lit("no"))).alias("lowestPVal"),
            F.flatten(F.collect_set(F.when(F.col("rightTissue") == "yes", F.col("isRightTissueSignalAgreed")))).alias("isRightTissueSignalAgreed"),
            F.collect_set(F.when(F.col("rightTissue") == "yes", F.col("isSignalFromRightTissue"))).alias("isSignalFromRightTissue"),
        )
...
for key, df in pivoted_dfs.items():
    columns_iterate=pivoted_dfs[key].drop(*to_drop).columns
    columns_keep=pivoted_dfs[key].select(*to_keep)
    print(key)
    print("transforming columns into string by assessing yes,no or null")
    pivoted_dfs_t[key] = pivoted_dfs[key].select(*columns_keep,
        *[
            F.when(F.size(F.col(col)) == 0, F.lit("no"))  # If empty array, set to "no"
            .when(F.array_contains(F.col(col), "yes"), F.lit("yes"))  # If contains "yes", set "yes"
            .when(F.array_contains(F.col(col), "no"), F.lit("no"))  # If contains "no", set "no"
            .otherwise(F.lit(None))  # Default case, to handle other unexpected values
            .alias(col) if col in columns_iterate[5:] else F.col(col)
            for col in columns_iterate[5:]
        ]
    )
    print("finished transformed datasets")

def process_pivoted_data(df_name):
    df = pivoted_dfs_t[df_name]  # Get the DataFrame
    df = df.cache()  # Cache for performance

    rows = comparisons_df_iterative(df.columns[7:])  # Get row comparisons

    results = []
    for row in rows:  # Iterate normally (avoiding map)
        res = aggregations_original(df, "propagated", listado, *row, today_date)
        res.append(df_name)  # Append DataFrame name
        results.append(res)  # Store results in a list
    
    return results
result_all=[]
# Process both DataFrames and extend `result_all`
result_all.extend(process_pivoted_data("projectId"))
result_all.extend(process_pivoted_data("biosampleName"))
result_all.extend(process_pivoted_data("rightStudyType"))
result_all.extend(process_pivoted_data("colocDoE"))

df = pd.DataFrame(result_all, columns=['columns','which','Phase','oddsRatio','sign','upperInterval','lowerInterval','total','values','relSuccess','upperRel','lowerRel','path','type'])
...
print("printing dataset and saving into google storage")
df.toPandas().to_csv(
