...
newColoc = (
    new.join(
        credible.selectExpr(  #### studyLocusId from credible set to uncover the codified variants on left side
            ...
            # "region",
            "beta as leftBeta",
            "pValueExponent as leftPValueExponent",
            "studyType",
        ).withColumnRenamed(
    ...
    .join(
        credible.selectExpr(  #### studyLocusId from credible set to uncover the codified variants on right side
            ...
            # "chromosome",
            # "region",
            "beta", #### to take the beta with the lowest pValue
            "pValueExponent",
            "studyType",
        )
...
gwasComplete = filtered_df.join(
    credible.selectExpr(
        "studyLocusId", "studyId", "variantId", "beta as betaGwas", "pValueExponent as pValueExponentGWAS"
    ),
    on="studyLocusId",
    how="left",
).persist()
...
resolvedColoc = (
    ...
    ### propagation was here
        ...
        F.when(
            ...
            F.when(
                (F.col("betaGwas") > 0) & (F.col("rightBeta") > 0), ### leftBeta and betGwas are the same
                F.lit("GoF_risk"),
            )
            .when(
                (F.col("betaGwas") > 0) & (F.col("rightBeta") < 0),
                F.lit("LoF_risk"),
            )
            .when(
                (F.col("betaGwas") < 0) & (F.col("rightBeta") > 0),
...
### Evidence Dataset Option 1: Right beta with lowes p Value
### at StudyLocusId - Target - Disease Level.

# Define the window specification
window_spec = Window.partitionBy("leftStudyLocusId","targetId","diseaseId").orderBy(F.col("pValueExponent").asc())
window_spec_ratio = Window.partitionBy("leftStudyLocusId","targetId","diseaseId")

# Add a new column that takes the value of D corresponding to the lowest E value
resolvedColoc3 = resolvedColoc.withColumn("Beta_with_lowest_pValue", F.first("rightBeta").over(window_spec)
        ).withColumn("qtlType_lowestPValue", F.first("rightStudyType").over(window_spec)
        ).withColumn("colocVariant_lowesPValue", F.first("colocVariant").over(window_spec)
        ).withColumn("betaRatio", F.collect_set("betaRatioSignAverage").over(window_spec_ratio))

######
### OPTION 1 lowest pa value evidences
######
### Building associations from lowesrt p value
builtOtEvidence_lowestBeta=discrepancifier(filtered_df.join(resolvedColoc3.withColumnRenamed("leftStudyLocusId","studyLocusId"
    ).groupBy("studyLocusId","betaGwas","targetId","diseaseId","Beta_with_lowest_pValue","colocVariant_lowesPValue","qtlType_lowestPValue"
    ).count().drop("count"), on=["studyLocusId","targetId","diseaseId"],how="left").persist())


######
### OPTION 2 using BetaSignAverageRatio
######
builtOtEvidence_betaRatio=discrepancifier(filtered_df.join(resolvedColoc3.withColumnRenamed("leftStudyLocusId","studyLocusId"
    ).groupBy("studyLocusId","betaGwas","targetId","diseaseId","betaRatio"
    ).pivot("colocDoE").count().drop("count"), on=["studyLocusId","targetId","diseaseId"],how="left").persist())
