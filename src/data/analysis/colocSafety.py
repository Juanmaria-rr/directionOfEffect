#### copy-paste here the code to run the analysis.
...
def comparisons_df() -> list:
    ...
    dataType = ["byDatatype"] * len(toAnalyze)
    l_studies = []
    l_studies.extend([list(a) for a in zip(toAnalyze, dataType)])
...
for row in aggSetups_original:
    aggregations_original(uniqueBetas_analyse, "propagated", listado, *row, today_date)

#### reading parquets and making spreadsheet
...
word_analysis = "safety"
print("writing csv file")
file_name = f"gs://ot-team/jroldan/analysis/{today_date}_{word_analysis}_analysis.csv"
df.to_csv(file_name)
